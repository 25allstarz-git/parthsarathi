-- Drop citizen_verifications table (Aadhaar verification replaced by mobile phone OTP auth)
DROP TABLE IF EXISTS public.citizen_verifications CASCADE;

-- Update register_role function to check phone confirmation from auth.users
...
BEGIN
  ...
  -- A stored phone is not proof of ownership; require Supabase Auth confirmation.
  SELECT EXISTS (
