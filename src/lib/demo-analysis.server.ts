/**
 * Deterministic fallback analysis adapter.
 *
 * Used only when the AI gateway is unavailable (no key, quota exhausted, upstream
 * error). It produces the same `AnalysisResult` shape the live model returns, built
 * from the citizen's own words, the chosen category and the uploaded file names, so
 * the intake → analysis → advocate journey is always demonstrable. Swapping in a real
 * provider requires no UI change.
 */
import type { AnalysisResult } from "./ai.server";
...
interface CategoryProfile {
  ...
  similar: Array<{ case_number: string; title: string; court: string; outcome: string; similarity: number }>;
}
...
const PROFILES: Record<string, CategoryProfile> = {
  "Criminal Law": {
    urgency: "critical",
    urgency_reason:
      "Criminal proceedings carry liberty consequences and strict statutory timelines for bail and remand.",
    ...
    insights: [
      {
        ...
        detail:
          "Where arrest is apprehended, a petition under Section 482 BNSS (formerly Section 438 CrPC) may be moved before the Sessions Court, supported by the documents on record.",
      },
      {
        heading: "Preserve the first information record",
        detail:
          "A certified copy of the FIR and case diary entries should be obtained early; delay in obtaining them weakens the ability to challenge procedural lapses.",
      },
    ],
    precedents: [
      {
  ...
  "Family Law": {
    urgency: "high",
    urgency_reason:
      "Matrimonial and custody matters affect maintenance and the welfare of dependants, which courts treat as time-sensitive.",
    ...
    insights: [
      {
        ...
        detail:
          "An application under Section 24 of the Hindu Marriage Act, 1955 or Section 144 BNSS permits interim maintenance while the main petition is pending.",
      },
      {
        heading: "Mediation is a mandatory first step",
        detail:
          "Family Courts ordinarily refer parties to the mediation centre before framing issues; a considered settlement position saves several hearings.",
      },
    ],
    precedents: [
      {
  ...
  "Consumer Protection": {
    urgency: "medium",
    urgency_reason:
      "A two-year limitation runs from the date the cause of action arose, leaving room for a properly prepared complaint.",
    ...
    insights: [
      {
        ...
        detail:
          "Each failure — delay, non-delivery or misrepresentation — should be pleaded with the corresponding invoice, booking letter or correspondence annexed.",
      },
      {
        heading: "Compensation with interest is routinely allowed",
        detail:
          "Commissions commonly award refund with interest from the date of payment, together with litigation costs.",
      },
    ],
    precedents: [
      {
    ...
    insights: [
      {
        ...
        detail:
          "An application under Order XXXIX Rules 1 and 2 CPC filed with the suit protects the status quo of the property pending trial.",
      },
      {
        heading: "Title chain must be assembled",
        detail:
          "Sale deed, mutation records, tax receipts and the encumbrance certificate together establish the chain of title.",
      },
    ],
    precedents: [
      {
    ...
    insights: [
      {
        ...
        detail:
          "The DAR filed by the police, together with the FIR, mechanical inspection report and medical records, drives the quantum of compensation.",
      },
      {
        heading: "Income proof determines the multiplier",
        detail:
          "Salary slips, income tax returns or a certificate of earnings should be placed on record to establish the annual income.",
      },
    ],
    precedents: [
      {
  ...
  "Cyber Law": {
    urgency: "critical",
    urgency_reason:
      "Digital evidence and intermediary logs are retained for limited periods; preservation requests are time-critical.",
    ...
    insights: [
      {
        ...
        detail:
          "A written request to the intermediary and the investigating officer under the IT Act, 2000 preserves logs before routine deletion.",
      },
      {
        heading: "Certificate under Section 63 BSA",
        detail:
          "Electronic records require the statutory certificate (formerly Section 65B of the Evidence Act) to be admissible.",
      },
    ],
    precedents: [
      {
  ...
  insights: [
    {
      ...
      detail:
        "A dated chronology with each document annexed is what a court reads first; it also shortens the advocate's preparation.",
    },
    {
      heading: "Check the limitation position",
      detail:
        "The Limitation Act, 1963 prescribes distinct periods for each type of claim; the period is computed from the date the cause of action arose.",
    },
  ],
  precedents: [
    {
...
function pickCategory(description: string, chosen?: string): string {
...
function toTitle(description: string, category: string): string {
  const firstSentence = description.split(/[.\n]/)[0]?.trim() ?? "";
  const trimmed = firstSentence.length > 12 ? firstSentence.slice(0, 78) : `${category} matter`;
  return trimmed.charAt(0).toUpperCase() + trimmed.slice(1);
}

function findDates(description: string): Array<{ date: string; event: string; source: string }> {
  const results: Array<{ date: string; event: string; source: string }> = [];
...
export function demoAnalysis(input: {
  ...
  return {
    summary: `This is a ${category.toLowerCase()} matter. ${
      sentences[0] ?? input.description.slice(0, 200)
    } Based on the material placed on record${
      docNames.length ? ` and the ${docNames.length} document(s) uploaded` : ""
    }, the matter has been classified as ${profile.urgency} urgency. ${profile.urgency_reason} The next practical step is to engage an advocate practising in ${profile.specializations[0]} so that the appropriate proceeding can be instituted before the ${profile.court}.`,
    category,
    ...
    parties: [
      { role: "Applicant", name: "The citizen filing this matter" },
      { role: "Respondent", name: "Opposite party named in the statement of facts" },
    ],
    legal_insights: profile.insights.map((i) => ({ ...i, source: primarySource })),
...
export function demoAnswer(question: string, audience: "citizen" | "lawyer" | "judge"): string {
  const opener =
    audience === "citizen"
      ? "Here is a plain-language response based on the record on file."
      : audience === "lawyer"
        ? "Here is a working note based on the record on file."
        : "Here is a neutral note based on the record on file.";

  return `${opener}

On your question — "${question.slice(0, 180)}" — the record currently holds your statement of facts, the documents you uploaded and the structured analysis generated at intake. The practical sequence is:

1. Confirm the chronology of events and make sure every date has a supporting document.
2. Check the limitation period applicable to this category of claim under the Limitation Act, 1963.
3. Identify the forum with jurisdiction and the relief you intend to seek.
4. Share the case with an advocate on ParthSarathi so the pleadings can be settled.

The detailed research assistant is temporarily unavailable, so this is a structured checklist rather than a model-generated opinion.

This is AI-assisted guidance only — it is not legal advice or a judicial decision.`;
}
